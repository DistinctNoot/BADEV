...
class moderation(commands.Cog):
	...
	async def kick(self, ctx, member: discord.User, *, reason=None):
		...
		channel = await member.create_dm()
		await channel.send(f"Kicked from {ctx.guild.name}. Reason: {reason}")
	...
	async def user(self, ctx, member: discord.Member = None):
		...
		embed = discord.Embed(
		    color=0x9E0B0B, timestamp=datetime.datetime.utcnow())
	...
	async def embedimg(self, ctx, *, msg):
		...
		em = discord.Embed(
		    description=
		    "{}\n[Join this server for updates](https://discord.gg/v9hgwZV)".
		    format(msg),
		    color=0x9E0B0B)
	...
	async def answer(self, ctx, user: discord.User):
		...
		em = discord.Embed(
		    title="Your application for BA developer has been accepted.",
		    color=0x9E0B0B)
		em.add_field(
		    name="Your application for developer has been accepted",
		    value=
		    "You will be given access to the [repl](https://repl.it/@BADEVTOOLS/BADEV#cogs/moderation.py) you will be given the trainee role and your code will be monitored"
		)
